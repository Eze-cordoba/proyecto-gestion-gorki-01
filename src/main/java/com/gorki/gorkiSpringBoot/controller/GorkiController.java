...
import com.gorki.gorkiSpringBoot.DTO.UsuarioDTO;
...
public class GorkiController {
    ...
    public ResponseEntity<?> verUsuario(@PathVariable Long idUsuario) {
        ...
        try {
            ...
            UsuarioDTO usuarioDto = new UsuarioDTO(usuarioBd);
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'DEPORTISTA')")
    @GetMapping("/mostrarReservasDeUsuario/{idUsuario}")
    public ResponseEntity<?> mostrarReservasDeUsuario(@PathVariable long idUsuario) {

        try {
            List<Reserva> reservas = usuarioService.findUsuario(idUsuario).getReservas();
            return ResponseEntity.ok(reservas);
        }catch (Exception e){
         return    ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("ERROR AL MOSTRAR LAS RESERVAS DEL USUARIO");
        }

    }

    @PreAuthorize("hasAnyRole('ADMIN', 'DEPORTISTA')")
    @GetMapping("/eliminarReservaUsuario/{idReserva}")
    public ResponseEntity<?> eliminarReservaDelUsuario(@PathVariable long idReserva) {
                ...
                if (ahora.isBefore(horario)) {
                    ...
                    for (Reserva reserva : deporteBd.getReservas()) {
                        ...
                        if (reserva.getFechaInicio().equals(horario)) {
                            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("el horario ya esta reservado");
                        }
